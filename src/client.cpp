#include "/home/shark/code/TinyDB/include/TinyClient.h"
#include <sys/types.h>

int TinyDBClient::createSocket()
{
...
sockaddr_in TinyDBClient::serverStructure()
{
...
bool TinyDBClient::connectionHandling(sockaddr_in serveraddr)
{
...
TinyDBClient::TinyDBClient() : sockfd(-1), connected(false) {}

TinyDBClient::~TinyDBClient()
{
	if (connected)
	{
		close(sockfd);
...
void TinyDBClient::TinyClientStart()
{
	try
	{
		sockfd = createSocket();
...
void TinyDBClient::sendReq(const std::string &req)
{
...
std::string TinyDBClient::recvIntoStr()
{
...
double TinyDBClient::recvIntoDouble()
{
...
float TinyDBClient::recvIntoFloat()
{
...
void TinyDBClient::recvIntoBin(std::string filename){
